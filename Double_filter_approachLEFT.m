...
for VF=1:numel(h5files)
% % %     if VF == 3
% % %         continue
% % %     end
    %% LOAD DATA
    sp='-';
    TT=strcat((h5files(VF).name),sp,'LEFT leg');
    TT=char(TT);
    File=strcat(h5files(VF).name);
    File=char(File);
    rx = hdf5read(File, '/SI-001202/Calibrated/Gyroscopes');
    rx=transpose(rx);
    rx=rx(:,2);
    x = rx;
...
xf1 = filter(b,a,x);
    ...
    a=transpose(a);
    for b=1:numel(locsf1)
        [pksto(b),locsto(b)]=findpeaks(a(:,b),'minpeakheight', 0,'minpeakdistance',1, 'npeaks',1);%'minpeakprominence', 0.1,
        hold on
    end
...
%         xf3(TOTf2-30),xf3(TOTf2-31),xf3(TOTf2-32),xf3(TOTf2-33));
    ...
    LEFTPREINISWms=(ALL(3,:)-ALL(1,:)*0.007825);
    LEFTTERMSWms=(ALL(2,:)*0.007825-ALL(3,:));
    ...
    pathfilename=strcat('C:\Users\mv100\OneDrive - University of Brighton\MATLAB\Articulation\DATA\',filenameonly,sp,'LeftLeg');

    save(pathfilename, 'LEFTHSms','LEFTTOTms','LEFTMSms', 'LEFTSWms','LEFTSTms', 'LEFTPREINISWms','LEFTSTRTms', 'LEFTTERMSWms');
